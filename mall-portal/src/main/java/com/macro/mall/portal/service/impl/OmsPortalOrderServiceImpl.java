...
public class OmsPortalOrderServiceImpl implements OmsPortalOrderService {
    ...
    public Integer paySuccess(Long orderId, Integer payType) {
        // 条件 UPDATE：仅 status=0 的订单可改为已付款，防止与 cancelOrder 并发冲突
    ...
    public Integer cancelTimeOutOrder() {
        ...
        // 逐个调用 cancelOrder（内含条件 UPDATE 防并发）
        int count = 0;
    ...
    public void cancelOrder(Long orderId) {
        // 先查出订单元数据（couponId, memberId, useIntegration 用于后续资源释放）
        OmsOrder order = orderMapper.selectByPrimaryKey(orderId);
        if (order == null || order.getStatus() != 0) {
            return;
        }
        // 条件 UPDATE：仅当 status=0 时才改为已关闭，防止与 paySuccess 并发冲突
        OmsOrderExample example = new OmsOrderExample();
        example.createCriteria().andIdEqualTo(orderId).andStatusEqualTo(0).andDeleteStatusEqualTo(0);
        OmsOrder updateOrder = new OmsOrder();
        updateOrder.setStatus(4);
        int rows = orderMapper.updateByExampleSelective(updateOrder, example);
        if (rows == 0) {
            return;
        }
    ...
    /**
     * 锁定下单商品库存（原子 UPDATE，WHERE 条件防超卖）
     */
    private void lockStock(List<CartPromotionItem> cartPromotionItemList) {
        for (CartPromotionItem cartPromotionItem : cartPromotionItemList) {
            int rows = portalOrderDao.lockSkuStock(
